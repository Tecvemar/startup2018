...
def start_server(root_path, port, netport, addons_path):
    os.system(
        'python %sopenerp-server.py -r %s -w %s  --pidfile=openerp.pid --config=%s --no-xmlrpcs --xmlrpc-port=%s --netrpc-port=%s --addons-path=%s' %
        (root_path, dbdata['postgresql_login'], dbdata['postgresql_password'], dbdata['config_file'], str(port), str(netport), addons_path))
def clean():
...
def wait(id,url=''):
    ...
    while not progress==1.0:
        progress,users = execute(sock2,'get_progress',dbdata['openerp_password'], id)
    return True


def create_db(uri, dbname, user, pwd, lang='es_VE'):
    ...
    if dbname in db_list:
        drop_db(uri, dbname)
    #~ Hbto
    id = execute(conn,'create',dbdata['openerp_password'], dbname, False, lang)
    #~ Hbto
    wait(id,uri)
    #~ Hbto
    install_module(uri, dbname, [],user=user,pwd=pwd)
    #~ Hbto
    return True
...
def drop_db(uri, dbname):
    ...
    db_list = execute(conn,'list')
    if dbname in db_list:
        execute(conn, 'drop', dbdata['openerp_password'], dbname)
    return True
...
def install_module(uri, dbname, modules, addons, extra_addons,  user, pwd):
    uid = login(uri, dbname, user, pwd)
    if extra_addons:
    ...
    if uid:
        ...
        execute(obj_conn, 'execute', dbname, uid, pwd, 'base.module.upgrade', 'upgrade_module',[])
        #~ HBTO
    return True

def upgrade_module(uri, dbname, modules, user, pwd):
    uid = login(uri, dbname, user, pwd)
    if uid:
        obj_conn = xmlrpclib.ServerProxy(uri + '/xmlrpc/object')
        wizard_conn = xmlrpclib.ServerProxy(uri + '/xmlrpc/wizard')
